...
export function wallAnchor(p, view = {}) {
  ...
  if (q.geom === 'whip') { const hpx = (q.size + 12) * k; return { x: 8 * k, y: hpx - 2 * k, axis: '-y' }; }
  if (q.geom === 'stub') { const hpx = (q.size + 6) * k; return { x: 6 * k, y: hpx - 2 * k, axis: '-y' }; }
  if (q.geom === 'patch') { const s2 = q.size * k; return { x: 2 * k + s2 * 0.5, y: s2 + 4 * k, axis: '-y' }; }
